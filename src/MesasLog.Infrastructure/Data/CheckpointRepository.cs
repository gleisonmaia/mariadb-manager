...
public sealed record BinlogCheckpoint(string File, long Position);

public sealed class CheckpointRepository(MariaDbConnectionFactory factory, ILogger<CheckpointRepository> logger)
{
    ...
    public async Task SaveAsync(string file, long position, CancellationToken ct = default)
    {
        ...
        logger.LogDebug("Checkpoint atualizado: {File} @{Pos}", file, position);
    }
}
